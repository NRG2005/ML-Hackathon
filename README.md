...
1. FastPatternMatcher: Filters a 50,000-word corpus against the current pattern (e.g., _ _ A _ _).
2. FastNGramModel: Uses unigram, bigram, and trigram statistics to predict the most likely letter in a blank.
3. FastFrequencyModel: Uses global and length-specific letter frequencies as a baseline.
4. Reinforcement Learning (DQN): A MicroDQN (a tiny deep Q-network) acts as a "meta-learner." It learns from its mistakes and successes to decide the optimal blend of the heuristic models at any given game state.
5. Interactive Frontend: A simple and "cool" Streamlit (app.py) app that lets you:
